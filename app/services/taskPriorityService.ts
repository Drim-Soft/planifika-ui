// Servicio para manejar prioridades de tareas
...
class TaskPriorityService {
  ...
  async getAllTaskPriorities(): Promise<TaskPriority[]> {
    return this.request<TaskPriority[]>('/task-priority');
  }

  async getTaskPriorityById(id: number): Promise<TaskPriority> {
    return this.request<TaskPriority>(`/task-priority/${id}`);
  }

  async getTaskPriorityByName(name: string): Promise<TaskPriority> {
    return this.request<TaskPriority>(`/task-priority/name/${name}`);
  }
}
